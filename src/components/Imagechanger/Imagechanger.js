...
const Imagechanger = () => {
  ...
  const text = [
    ...
    "lorem ipsum dolor sit amet, consectetur adipisci dolor sit amet, consectetur adipisci dolor sit amet, consectetur adipisci dolor sit amet, consectetur adipisci dolor sit amet, consectetur adipiscing elit,",
  ];
  ...
  const toggleSection = (idx) => {
    ...
    newExpandedSections[idx] = !newExpandedSections[idx];
  ...
  return (
    <>
      <div className="main w-full h-120 bg-gray-100 rounded-[50px] flex p-2">
        <div className="left text-container flex flex-col items-center justify-center w-1/2 h-full pt-2">
          {Heading.map((value, idx) => (
            <div key={idx} className='mb-7'>
              {idx === 1 && <div className="line-horizontal dark-black-line h-1 w-full bg-gray-300" />}
              <div onClick={() => toggleSection(idx)} className='flex items-center justify-between'>
                <h1 className="text-2xl my-4 font-bold text-left cursor-pointer">
                  {value}
                </h1>
                {activeIndex === idx ? <FaChevronUp className='text-2xl my-4 font-sans text-gray-500 text-left cursor-pointer'/> : <FaChevronDown className='text-2xl my-4 text-gray-500 text-left cursor-pointer'/>}
              </div>
              <div className='w-[23rem]'>
                <p className={`${idx + 1}text`}>{expandedSections[idx] ? text[idx] : ""}</p>
              </div>
        ...
        <div className="right image-container flex w-1/2 h-full p-1">
          <div className="flex w-full h-full justify-center items-center">
            <img src={img[activeIndex !== null ? activeIndex : 0]} alt="" className="rounded-[50px] w-[80%] h-[72%]" />
          </div>
        </div>
      </div>
      <br/><br/>
